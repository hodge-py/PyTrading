...
class pyStock:
    ...
    def calculate_fundamentals(self):
        ...
        pe_fwd = f"{info.get('forwardPE', 'N/A'):.2f}" if info.get('forwardPE', None) else 'N/A'
        current_ratio = f"{info.get('currentRatio', 'N/A'):.2f}" if info.get('currentRatio', None) else 'N/A'
        debt_to_equity = f"{info.get('debtToEquity', 'N/A'):.2f}" if info.get('debtToEquity', None) else 'N/A'
        return_on_equity = f"{info.get('returnOnEquity', 'N/A'):.2f}" if info.get('returnOnEquity', None) else 'N/A'
        earnings_per_share = f"${info.get('earningsPerShare', 'N/A'):.2f}" if info.get('earningsPerShare', None) else 'N/A'
        price_to_book = f"{info.get('priceToBook', 'N/A'):.2f}" if info.get('priceToBook', None) else 'N/A'
        beta = f"{info.get('beta', 'N/A'):.2f}" if info.get('beta', None) else 'N/A'
    ...
    def calculate_technicals(self):
        sma_20 = f"{ta.sma(self.data['Close'], length=20).iloc[-1]:.2f}"
        sma_50 = f"{ta.sma(self.data['Close'], length=50).iloc[-1]:.2f}"
        rsi_14 = f"{ta.rsi(self.data['Close'], length=14).iloc[-1]:.2f}"
        ...
        self.df_tech = pd.DataFrame({
            ...
            'Value': [sma_20, sma_50, rsi_14, f"{macd.iloc[-1,0]:.2f}", f"{macd.iloc[-1,1]:.2f}", f"{macd.iloc[-1,2]:.2f}"]
        })
        return self.df_tech
    
    def sma_strategy(self):
        self.data['SMA 20'] = ta.sma(self.data['Close'], length=20)
        self.data['SMA 50'] = ta.sma(self.data['Close'], length=50)
        self.data['Signal'] = 0
